...
def sgd(lr, tparams, grads, x, mask, y, cost):
    ...
    return f_grad_shared, f_update
def read_data():
    fMRI_file = open('dfnc_cluster_5.csv', 'r')
    sMRI_file = open('sMRI_loading.csv', 'r')   
...
def train(dim_word=4, # word vector dimensionality
    ...
    for run_no in range(100):
        ...
        saveto = 'Results/model_run%d.txt' %(run_no + 1)    
        params = unzip(tparams)
        numpy.savez(saveto, **params)
        
        use_noise.set_value(0)
        a_file = open('Results/alignment_run%d.txt' %(run_no + 1), 'w')
        for x,y in data:
            x, x_mask, y, y_mask = prepare_data([x], [y])
